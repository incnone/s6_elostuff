...
def elo_to_gamma(elo: float) -> float:
    return pow(10.0, elo/400.0)


def gamma_to_elo(gamma: float) -> float:
    return 400*math.log10(gamma)
...
class Player(object):
    def __init__(self, name, prior_elo=0.0, prior_stdev=400.0):
        self.name = name
        self.prior_r = elo_to_r(prior_elo)
        self.prior_var = elo_to_r(prior_stdev)**2
        self.last_elo = prior_elo
        self.gamma = elo_to_gamma(prior_elo)
    ...
    @property
    def elo(self):
        return gamma_to_elo(self.gamma)

    @property
    def log_likelihood(self):
        r = self.r
        first = self._total_wins * r
        second = 0.0
        for player, matchup in self._matchups.items():
            second += matchup.wins * math.log(self.gamma + player.gamma)
        third = (r - self.prior_r)**2 / (2*self.prior_var)
        return first - second - third

    @property
    def test_log_likelihood(self):
        r = self.r
        first = self._total_test_wins * r
        second = 0.0
        for player, matchup in self._test_matchups.items():
            second += matchup.wins * math.log(self.gamma + player.gamma)
        return first - second

    @property
    def grad_component(self):
        first = self._total_wins / self.gamma
        second = 0
        for player, matchup in self._matchups.items():
    ...
    def run_mm_iteration(self):
        self.last_elo = gamma_to_elo(self.gamma)

        a = self._total_wins
        c = 1.0/self.prior_var
        b = 0
        m = self.prior_r
        for player, matchup in self._matchups.items():
            b += matchup.games/(self.gamma + player.gamma)

        if b == 0:  # No games played
            return

        # NB argument is > 0, so principle branch of lambert W is real
        warg = (b / c) * mpmath.exp((a + m * c) / c)
        self.gamma = (c/b)*mpmath.lambertw(warg)
    ...
    with open('elorate_log.txt', 'w') as iter_log:
        for idx in range(max_repeats):
            ...
            for player in player_list:
                ...
                total_gamma += player.gamma

            for player in player_list:
                l2_elo_diff += (player.elo - player.last_elo)**2
            ...
            if l2_elo_diff < 0.000000000001 and l2_grad < 0.000000000001:
